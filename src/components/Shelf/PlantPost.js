...
const PlantPost = (props) => {
  ...
  useEffect(() => {
    axios.get(`https://ecourse.cpe.ku.ac.th/exceed05/api/customer/plant_detail/${props.id}`)
    .then((response) => {
  ...
  axios.post("https://ecourse.cpe.ku.ac.th/exceed05/api/customer/reserve",data,headers)
  .then((reponse)=>{
    console.log("success");
  })
  .catch((error)=>{
  ...
  const checkLogin = (props) => {
    if(props.booking == 0 && localStorage.getItem("token")){
      console.log("sssssss");
      return(
